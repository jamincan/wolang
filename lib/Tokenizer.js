...
export default class Tokenizer {
  ...
  next() {
    ...
    if (this.atLineStart()) {
      ...
      if (string.startsWith('\t')) {
        const [line, col] = this.getXY();
        throw SyntaxError(`Unexpected token at [${line}, ${col}]: "\\t"`);
      }
      ...
      if (diff !== 0) {
        const [line, col] = this.getXY();
        throw SyntaxError(`Unexpected indent of ${lineIndent} at [${line}, ${col}].`);
      }
    ...
    const [line, col] = this.getXY();
    throw SyntaxError(`Unexpect token at [${line}, ${col}]: "${string[0]}"`);
  }
  ...
  // Returns the line number and column at the given position
  getXY(pos = this.cursor) {
    // Get count of newlines before pos
    const line = (/\n/g.exec(this.string.slice(0, pos)) || []).length;
    const column = this.cursor - this.lineStart(pos);
    return [line, column];
  }
