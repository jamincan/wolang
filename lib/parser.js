...
export default class Parser {
  ...
  /** Set -> returns a repeating list of Intervals
   *  | Repeat NewLine INDENT Set... DEDENT
   *  : Repeat IntervalList
   */
  Set() {
    const repeat = this.Repeat();
    // NewLine after a repeat should indicate a set
    if (this.NewLine()) {
      this.queue.eat('INDENT');
      let sets = [this.Set()];
      // Continue grabbing sets until a DEDENT is reached
      while (!this.queue.optional('DEDENT')) sets.push(this.Set());
      return Set(repeat, intervals);
    }
    const sets = this.IntervalList();
    return Set(repeat, sets);
  }
  ...
  NewLine() {
    ...
    let string = '';
    let next;
    while ((next = this.queue.optional('NEWLINE'))) string += next;
